...
export default function GalleryTile({ destination }: { destination: string }) {
    return (
        <motion.div
            ...
            className="w-full"
        >
            <h3 className="text-white font-serif font-bold text-2xl mb-6 pl-2">Hidden Gems in {destination}</h3>

            <div className="flex gap-4 overflow-x-auto pb-8 snap-x scrollbar-hide">
                {MOCK_IMAGES.map((src, idx) => (
                    <div key={idx} className="relative min-w-[250px] h-[300px] rounded-[1.5rem] overflow-hidden snap-center group cursor-pointer border border-white/5">
                        <Image
                            src={src}
                            alt="Gem"
                            fill
